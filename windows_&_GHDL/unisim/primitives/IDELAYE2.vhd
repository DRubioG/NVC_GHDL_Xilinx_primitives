...
------------------------------------------------
  ...
  begin
    if((refclk_freqncy <= 410.0) and (refclk_freqncy >= 390.0)) then
       return 39;
    elsif ((refclk_freqncy <= 310.0) and (refclk_freqncy >= 290.0)) then 
      return 52;
    else
      return 78;
    end if;
...
-------------------- constants --------------------------
  ...
  constant	MAX_REFCLK_FREQUENCYH   : real := 410.0;
...
-------------- variable declaration -------------------------
  ...
  signal IS_DATAIN_INVERTED_BIN : std_ulogic;
  signal IS_IDATAIN_INVERTED_BIN : std_ulogic;
...
begin
  ...
  DATAIN_dly     	 <= DATAIN xor IS_DATAIN_INVERTED_BIN        	after 0 ps;
  IDATAIN_dly    	 <= IDATAIN xor IS_IDATAIN_INVERTED_BIN       	after 0 ps;
  ...
  IS_DATAIN_INVERTED_BIN <= TO_X01(IS_DATAIN_INVERTED);
  IS_IDATAIN_INVERTED_BIN <= TO_X01(IS_IDATAIN_INVERTED);
...
--####################################################################
 ...
 --    if((REFCLK_FREQUENCY < MIN_REFCLK_FREQUENCYL) or (REFCLK_FREQUENCY > MAX_REFCLK_FREQUENCYH)) then
     if((REFCLK_FREQUENCY >= 190.0 and REFCLK_FREQUENCY <= 210.0) or
        (REFCLK_FREQUENCY >= 290.0 and REFCLK_FREQUENCY <= 310.0) or
        (REFCLK_FREQUENCY >= 390.0 and REFCLK_FREQUENCY <= 410.0)) then
	null;
     else
         assert false
         report "Attribute Syntax Error: Legal values for REFCLK_FREQUENCY are either between 190.0 and 210.0, or between 290.0 and 310.0 or between 390.0 and 410.0"
         severity Failure;
     end if;
       ...
       if((GSR_dly = '1') or (FIRST_TIME))then
          ...
          if(rising_edge(C_in)) then
	       ...
	       if(PIPE_SEL = "TRUE") then
                 ...
                 idelay_count_var := Idelaye2_InitDelayCount(IDELAY_TYPE, IDELAY_VALUE, CNTVALUEIN_dly);
	       end if;
...
--####################################################################
    ...
    DATAOUT_zd     <= transport tap_out after INIT_DELAY ;
