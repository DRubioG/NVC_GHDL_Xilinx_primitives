...
architecture IOBUFDS_INTERMDISABLE_V of IOBUFDS_INTERMDISABLE is
  ...
  constant MODULE_NAME : string := "IOBUFDS_INTERMDISABLE";
  ...
  begin
    -------- DIFF_TERM check
      if((xil_attr_test) or
        ...
        Write ( Message, string'(IOBUFDS_INTERMDISABLE_V'PATH_NAME));
...
begin
  ...
  Behavior : process (IBUFDISABLE_in, IO_in, IOB_in, I_in, T_in, USE_IBUFDISABLE_BIN, DQS_BIAS_BIN)
     variable NOT_T_OR_IBUFDISABLE   : std_ulogic := '0';
  begin
    NOT_T_OR_IBUFDISABLE := ((IBUFDISABLE_in or (not T_in)) and USE_IBUFDISABLE_BIN);
