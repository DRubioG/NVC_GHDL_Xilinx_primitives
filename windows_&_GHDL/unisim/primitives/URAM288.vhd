...
function bit_and (
   ...
   variable v_bit_and : std_ulogic := '1';
begin
    ...
    if (glblGSR = '1') then
      ...
      if (a_sleep = '1' or DEEPSLEEP_in = '1' or SLEEP_in = '1' or auto_sleep = '1' or
        ...
        if (EN_ECC_WR_A_BIN = EN_ECC_WR_A_TRUE) then
          ...
          ram_data_a (71 downto 64) <= fn_ecc (c_encode , DIN_A_int (63 downto 0) , DIN_A_int (71 downto 64));
        elsif (EN_ECC_RD_A_BIN = EN_ECC_RD_A_TRUE) then
          ram_data_a (63 downto 0) <=
               (DIN_A_int (63) &
                (DIN_A_int (62) xor (INJECT_DBITERR_A_int)) &
    ...
    if (glblGSR = '1') then
      ...
      if (a_sleep = '1' or DEEPSLEEP_in = '1' or SLEEP_in = '1' or auto_sleep = '1' or
        ...
        if (EN_ECC_WR_B_BIN = EN_ECC_WR_B_TRUE) then
          ...
          ram_data_b (71 downto 64) <= fn_ecc (c_encode , DIN_B_int (63 downto 0) , DIN_B_int (71 downto 64));
        elsif (EN_ECC_RD_B_BIN = EN_ECC_RD_B_TRUE) then
          ram_data_b (63 downto 0) <=
               (DIN_B_int (63) &
                (DIN_B_int (62) xor (INJECT_DBITERR_B_int)) &
...
--  this causes X in douta doutb
--  process (INIT_RAM, shut_down)
--  begin
...
--  end process; 
  ...
  ram_data_a_out_p : process begin
    ...
    wait on RST_A_sync, RST_A_async, glblGSR, SLEEP_in, a_sleep, shut_down, ram_data_a_out;  --zh to add ram_ce_a ram_we_a
  end process;
  ...
  process (ram_we_a, ram_we_b, ram_ce_a, ram_ce_b, a_sleep, shut_down, ram_addr_a, ram_addr_b, ram_data_a, ram_data_b, ram_bwe_a, ram_bwe_b, ram_we_a_event, ram_we_b_event, RST_A_async, RST_B_async, RST_A_sync, RST_B_sync, glblGSR, INIT_RAM)
  ...
  begin
    ...
    --wr a
    --write like uram288 verilog
    --        ram_ce_a_rst := ram_ce_a and not RST_A_in;
    --        if (RST_A_async = '1' or RST_A_sync = '1' or shut_down = '1' or glblGSR = '1') then
    --          ram_data_a_out <= (others => '0');
    --        end if;
    --        if (ram_we_a = '1' and shut_down = '0' and a_sleep = '0' and glblGSR = '0') then
    ...
    --        end if;
    --        if ((ram_ce_a_rst = '1') and (ram_we_a = '0') and (shut_down = '0') and (a_sleep = '0') and (glblGSR = '0')) then -- rd a
    ...
    --        end if;
    --        if (ram_we_b = '1' and shut_down = '0' and a_sleep = '0' and glblGSR = '0') then
    ...
    --        end if;
    --        if ((ram_ce_b_rst = '1') and (ram_we_b = '0') and (shut_down = '0') and (a_sleep = '0') and (glblGSR = '0')) then -- rd b
    --          ram_data_b_out <= mem (to_integer(ram_addr_b));
    --        end if;
    -- verilog uram is written in sequential blocking , the vhdl need to be written to follow verilog sequential blocking way
    ...
    if (glblGSR = '1' or shut_down = '1') then
      ...
      if (RST_B_in = '1') then
        ...
        -- a test case had we_b on ,but we_a off 
        if (ram_we_b = '1') then -- wr b
          mem_ram_addr_b_v := (ram_data_b and ram_bwe_b) or
                              (mem (to_integer(ram_addr_b)) and not ram_bwe_b);
          mem_ram_addr_b <= mem_ram_addr_b_v;

          mem (to_integer(ram_addr_b)) <=  mem_ram_addr_b_v; -- wr mem b
        end if;

        --elsif (ram_we_a = '1') then -- wr a
        if (ram_we_a = '1') then -- wr a
          mem_ram_addr_a_v := (ram_data_a and ram_bwe_a) or
                              (mem (to_integer(ram_addr_a)) and not ram_bwe_a);
          mem_ram_addr_a <= mem_ram_addr_a_v;
          if (ram_we_b = '1') then -- wr b , did not work when we_b on, we_a off
            if (addr_match_v) then -- over write
              ...
              mem (to_integer(ram_addr_a)) <=  mem_ram_addr_a_v; -- wr mem a
              mem (to_integer(ram_addr_b)) <=  mem_ram_addr_b_v; -- wr mem b
            end if;
