...
-- INIT_FILE
       ...
       for d in 0 to tmp_mem_depth-1 loop
         ...
         for f_w in 0 to tmp_mem_width-tmp_memp_width-1 loop
           mem(d*(tmp_mem_width-tmp_memp_width)+f_w) <= tmp_mem_tmp(f_w);
         end loop;
         if (tmp_memp_width > 0) then
           for f_w2 in 0 to tmp_memp_width-1 loop
             memp(d*tmp_memp_width+f_w2) <= tmp_mem_tmp(tmp_mem_width-tmp_memp_width+f_w2);
           end loop;
         end if;
       end loop;
-- INIT_FILE
