...
------------------------------------------------
  ...
  begin
       if(DelayFormat = "TIME") then  
          if(SimDevice = "ULTRASCALE") then
           return integer(real(DelayValue)/5.0);
	  else
           return integer(real(DelayValue)/4.0);
	  end if;
...
-------------- variable declaration -------------------------
    signal	idelay_count_pre       : integer := Idelaye3_InitDelayCount(DELAY_FORMAT,DELAY_VALUE,SIM_DEVICE);
    signal	idelay_count_sync       : integer := Idelaye3_InitDelayCount(DELAY_FORMAT,DELAY_VALUE,SIM_DEVICE);
    signal	idelay_count_async       : integer := Idelaye3_InitDelayCount(DELAY_FORMAT,DELAY_VALUE,SIM_DEVICE);
    signal	cntvalue_updated       : real := real(Idelaye3_InitDelayCount(DELAY_FORMAT,DELAY_VALUE,SIM_DEVICE));
    signal	cntvalue_updated_sync       : real := real(Idelaye3_InitDelayCount(DELAY_FORMAT,DELAY_VALUE,SIM_DEVICE));
    signal	cntvalue_updated_async       : real := real(Idelaye3_InitDelayCount(DELAY_FORMAT,DELAY_VALUE,SIM_DEVICE));
...
--####################################################################
  ...
  variable idelay_count_var : integer := Idelaye3_InitDelayCount(DELAY_FORMAT,DELAY_VALUE,SIM_DEVICE);
...
-- CR 595286
	  ...
	  cntvalue_updated_async <= real(idelay_count_pre);
          FIRST_TIME   := false;
