...
-- Parameter encodings and registers
    ...
    constant INIT_BIN : std_logic_vector (71 downto 0) := INIT;
    ...
    constant SRVAL_BIN : std_logic_vector (71 downto 0) := SRVAL;
    ...
    constant c_encode                          : boolean := true;
...
function bit_or (
   ...
   variable v_bit_or : std_ulogic := '0';
begin
...
function fn_ecc (
   ...
   variable v_fn_ecc : std_logic_vector(7 downto 0);
begin
   v_fn_ecc(0) := d_i(0)  xor d_i(1)  xor d_i(3)  xor d_i(4)  xor d_i(6)  xor
   ...
   v_fn_ecc(1) := d_i(0)  xor d_i(2)  xor d_i(3)  xor d_i(5)  xor d_i(6)  xor
   ...
   v_fn_ecc(2) := d_i(1)  xor d_i(2)  xor d_i(3)  xor d_i(7)  xor d_i(8)  xor
   ...
   v_fn_ecc(3) := d_i(4)  xor d_i(5)  xor d_i(6)  xor d_i(7)  xor d_i(8)  xor
   ...
   v_fn_ecc(4) := d_i(11) xor d_i(12) xor d_i(13) xor d_i(14) xor d_i(15) xor
   ...
   v_fn_ecc(5) := d_i(26) xor d_i(27) xor d_i(28) xor d_i(29) xor d_i(30) xor
   ...
   v_fn_ecc(6) := d_i(57) xor d_i(58) xor d_i(59) xor d_i(60) xor d_i(61) xor
                d_i(62) xor d_i(63);
   ...
   if (encode) then 
      v_fn_ecc(7) := ecc_7 xor
                   v_fn_ecc(0) xor v_fn_ecc(1) xor v_fn_ecc(2) xor v_fn_ecc(3) xor
                   v_fn_ecc(4) xor v_fn_ecc(5) xor v_fn_ecc(6);
   else
      v_fn_ecc(7) := ecc_7 xor
...
-- full/empty variables
   ...
   --vc signal prog_full_val : unsigned (COUNT_WIDTH-1 downto 0) := (others => '0');
   signal prog_full_val_int : integer := 8192;


   --vc  constant COUNT_WIDTH : integer := 14;
   --vc vc_val2[12:0]= 1FFF = 8191 need increase width by 1-bit to comport value 8192
   --vc vc_val2[13:0]= 3FFF = 16383
   signal vc_val : unsigned (COUNT_WIDTH-1 downto 0) := (others => '0');
   signal vc_val2 : unsigned (COUNT_WIDTH-1 downto 0) := (others => '0');
   ...
   signal INIT_BIN_8_8     : std_logic_vector (7 downto 0) := (others => '0');
   signal INIT_BIN_7_0_8   : std_logic_vector (63 downto 0) := (others => '0');
   signal INIT_BIN_17_16_4 : std_logic_vector (7 downto 0) := (others => '0');
   signal INIT_BIN_15_0_4  : std_logic_vector (63 downto 0) := (others => '0');
   signal INIT_BIN_35_32_2 : std_logic_vector (7 downto 0) := (others => '0');
   signal INIT_BIN_31_0_2  : std_logic_vector (63 downto 0) := (others => '0');
   signal SRVAL_BIN_8_8     : std_logic_vector (7 downto 0) := (others => '0');
   signal SRVAL_BIN_7_0_8   : std_logic_vector (63 downto 0) := (others => '0');
   signal SRVAL_BIN_17_16_4 : std_logic_vector (7 downto 0) := (others => '0');
   signal SRVAL_BIN_15_0_4  : std_logic_vector (63 downto 0) := (others => '0');
   signal SRVAL_BIN_35_32_2 : std_logic_vector (7 downto 0) := (others => '0');
   signal SRVAL_BIN_31_0_2  : std_logic_vector (63 downto 0) := (others => '0');
...
--    CASNXTRDEN_in <= CASNXTRDEN;
  ...
  mem_rd_en_a  <= (RDEN_in or 
                  ((fill_lat or fill_reg or fill_ecc) and not SLEEP_A_int))
...
--    WRITE_WIDTH_4;
    ...
    -------- CASCADE_ORDER check
  if((xil_attr_test) or
     ((CASCADE_ORDER /= "NONE") and 
      (CASCADE_ORDER /= "FIRST") and 
      (CASCADE_ORDER /= "LAST") and 
      (CASCADE_ORDER /= "MIDDLE") and 
      (CASCADE_ORDER /= "PARALLEL"))) then
    attr_err := true;
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-101] CASCADE_ORDER attribute is set to """));
    Write ( Message, string'(CASCADE_ORDER));
    Write ( Message, string'(""". Legal values for this attribute are "));
    Write ( Message, string'("""NONE"", "));
    Write ( Message, string'("""FIRST"", "));
    Write ( Message, string'("""LAST"", "));
    Write ( Message, string'("""MIDDLE"" or "));
    Write ( Message, string'("""PARALLEL"". "));
    Write ( Message, string'("Instance "));
    Write ( Message, string'(FIFO36E2_V'PATH_NAME));
  ...
  if((xil_attr_test) or
    ...
    case READ_WIDTH is
      ...
      when others  =>  assert FALSE report "Error [Unisim FIFO36E2-119]: READ_WIDTH is not 4, 9, 18, 36 or 72." severity error;
    end case;
    case WRITE_WIDTH is
      ...
      when others  =>  assert FALSE report "Error [Unisim FIFO36E2-124]: WRITE_WIDTH is not 4, 9, 18, 36 or 72." severity error;
    end case;
    case READ_WIDTH is
      when  4   =>
      ...
      when others  =>  assert FALSE report "Error [Unisim FIFO36E2-130]: READ_WIDTH is out of range in PROG_EMPTY_THRESH check" severity error;
    end case;
    case WRITE_WIDTH is
      when  4   =>
      ...
      when  72   =>
        if (PROG_FULL_THRESH >= 1) and (PROG_FULL_THRESH <= mem_depth_f/16) then 
          ...
          Write ( Message, string'("Error : [Unisim "));
          Write ( Message, string'(MODULE_NAME));
          Write ( Message, string'("-135] PROG_FULL_THRESH attribute is set to "));
          Write ( Message, PROG_FULL_THRESH);
          Write ( Message, string'(". For WRITE_WIDTH = "));
          Write ( Message, WRITE_WIDTH);
          Write ( Message, string'(" PROG_FULL_THRESH must be in the range 1 to "));
      ...
      when others  =>  assert FALSE report "Error [Unisim FIFO36E2-136]: WRITE_WIDTH is out of range in PROG_FULL_THRESH check" severity error;
    end case;
    if  (attr_err) then
   ...
   RDEN_ecc   <= (RDEN_in or fill_reg ) when (EN_ECC_PIPE_BIN = EN_ECC_PIPE_TRUE) else '0';
   WREN_ecc   <= (RDEN_in or fill_reg or fill_ecc ) and not o_lat_empty and first_read 
                  when (EN_ECC_PIPE_BIN = EN_ECC_PIPE_TRUE) else '0';
   ...
   mem_a_mux  <= mem_a_reg when ((REGISTER_MODE_BIN = REGISTER_MODE_REGISTERED) or
                     (REGISTER_MODE_BIN = REGISTER_MODE_DO_PIPELINED)) else mem_a_ecc;
   memp_a_mux <= memp_a_reg when ((REGISTER_MODE_BIN = REGISTER_MODE_REGISTERED) or
                     (REGISTER_MODE_BIN = REGISTER_MODE_DO_PIPELINED)) else memp_a_ecc;
  ECCPARITY_out <= eccparity_reg;
   ...
   dbit_ecc    <= dbit_pipe when (EN_ECC_PIPE_BIN = EN_ECC_PIPE_TRUE) else dbit_lat;
   sbit_ecc    <= sbit_pipe when (EN_ECC_PIPE_BIN = EN_ECC_PIPE_TRUE) else sbit_lat;
   DBITERR_out <= dbit_reg when ((REGISTER_MODE_BIN = REGISTER_MODE_REGISTERED) or
                     (REGISTER_MODE_BIN = REGISTER_MODE_DO_PIPELINED)) else dbit_ecc;
   SBITERR_out <= sbit_reg when ((REGISTER_MODE_BIN = REGISTER_MODE_REGISTERED) or
                     (REGISTER_MODE_BIN = REGISTER_MODE_DO_PIPELINED)) else sbit_ecc;
   INIT_BIN_8_8     <= INIT_BIN(8) & INIT_BIN(8) & INIT_BIN(8) & INIT_BIN(8) &
                       INIT_BIN(8) & INIT_BIN(8) & INIT_BIN(8) & INIT_BIN(8);
   INIT_BIN_7_0_8   <= INIT_BIN(7 downto 0)   & INIT_BIN(7 downto 0) &
                       INIT_BIN(7 downto 0)   & INIT_BIN(7 downto 0) &
                       INIT_BIN(7 downto 0)   & INIT_BIN(7 downto 0) &
                       INIT_BIN(7 downto 0)   & INIT_BIN(7 downto 0);
   INIT_BIN_17_16_4 <= INIT_BIN(17 downto 16) & INIT_BIN(17 downto 16) & 
                       INIT_BIN(17 downto 16) & INIT_BIN(17 downto 16);
   INIT_BIN_15_0_4  <= INIT_BIN(15 downto 0)  & INIT_BIN(15 downto 0) &
                       INIT_BIN(15 downto 0)  & INIT_BIN(15 downto 0);
   INIT_BIN_35_32_2 <= INIT_BIN(35 downto 32) & INIT_BIN(35 downto 32);
   INIT_BIN_31_0_2  <= INIT_BIN(31 downto 0)  & INIT_BIN(31 downto 0);
   INIT_A_int <=
    (INIT_BIN_8_8     & INIT_BIN_7_0_8)  when (READ_WIDTH_BIN <= READ_WIDTH_9) else
    (INIT_BIN_17_16_4 & INIT_BIN_15_0_4) when (READ_WIDTH_BIN = READ_WIDTH_18) else
    (INIT_BIN_35_32_2 & INIT_BIN_31_0_2) when (READ_WIDTH_BIN = READ_WIDTH_36) else
     INIT_BIN;
   SRVAL_BIN_8_8     <= SRVAL_BIN(8) & SRVAL_BIN(8) & SRVAL_BIN(8) & SRVAL_BIN(8) &
                        SRVAL_BIN(8) & SRVAL_BIN(8) & SRVAL_BIN(8) & SRVAL_BIN(8);
   SRVAL_BIN_7_0_8   <= SRVAL_BIN(7 downto 0)   & SRVAL_BIN(7 downto 0) &
                        SRVAL_BIN(7 downto 0)   & SRVAL_BIN(7 downto 0) &
                        SRVAL_BIN(7 downto 0)   & SRVAL_BIN(7 downto 0) &
                        SRVAL_BIN(7 downto 0)   & SRVAL_BIN(7 downto 0);
   SRVAL_BIN_17_16_4 <= SRVAL_BIN(17 downto 16) & SRVAL_BIN(17 downto 16) & 
                        SRVAL_BIN(17 downto 16) & SRVAL_BIN(17 downto 16);
   SRVAL_BIN_15_0_4  <= SRVAL_BIN(15 downto 0)  & SRVAL_BIN(15 downto 0) &
                        SRVAL_BIN(15 downto 0)  & SRVAL_BIN(15 downto 0);
   SRVAL_BIN_35_32_2 <= SRVAL_BIN(35 downto 32) & SRVAL_BIN(35 downto 32);
   SRVAL_BIN_31_0_2  <= SRVAL_BIN(31 downto 0)  & SRVAL_BIN(31 downto 0);
   SRVAL_A_int <=
    (SRVAL_BIN_8_8     & SRVAL_BIN_7_0_8)  when (READ_WIDTH_BIN <= READ_WIDTH_9) else
    (SRVAL_BIN_17_16_4 & SRVAL_BIN_15_0_4) when (READ_WIDTH_BIN = READ_WIDTH_18) else
    (SRVAL_BIN_35_32_2 & SRVAL_BIN_31_0_2) when (READ_WIDTH_BIN = READ_WIDTH_36) else
     SRVAL_BIN;
...
-- CLOCK DOMAINS COMMON what is important is the result of the next clock edge.
   ...
   o_stages_empty <= ram_empty   when  (output_stages=0) else
                     ...
                     o_reg_empty when  (output_stages=3) else -- 3 FWFT + ECC + REG
                     o_reg_empty when ((output_stages=2) and (EN_ECC_PIPE_BIN = EN_ECC_PIPE_FALSE)) else -- 2 FWFT + REG
                     o_ecc_empty; -- 2 FWFT + ECC or 2 REG + ECC
...
-- Empty on external read and prev stage also empty
   ...
   CCE  : process (mem_rd_clk_a) begin
     if (rising_edge(mem_rd_clk_a)) then
       if ((glblGSR = '1') or (RDRST_int = '1')) then
          o_ecc_empty <= '1';
       elsif ((RDEN_ecc = '1') or (WREN_ecc = '1')) then
   ...
   CCF  : process (mem_rd_clk_a) begin
     if (falling_edge(mem_rd_clk_a)) then
       if ((glblGSR = '1') or (RDRST_int = '1') or (SLEEP_A_int = '1')) then
          fill_ecc <= '0';
       elsif ((o_lat_empty = '0') and (o_ecc_empty = '1') and (first_read = '1') and
              ((EN_ECC_PIPE_BIN = EN_ECC_PIPE_TRUE) and
               ((REGISTER_MODE_BIN = REGISTER_MODE_REGISTERED) or
                (FIRST_WORD_FALL_THROUGH_BIN = FIRST_WORD_FALL_THROUGH_TRUE)))) then
          fill_ecc <= '1';
       else
          fill_ecc <= '0';
...
-- full flag
 ...
 --prog_full_val_int <= mem_depth_f - (PROG_FULL_THRESH_BIN*wr_loops_b/4) + m_full;
   prog_full_val_int <= mem_depth_f - (PROG_FULL_THRESH_BIN*wr_loops_b/4);

   vc_val            <= to_unsigned(m_full,COUNT_WIDTH);
   vc_val2           <= to_unsigned(prog_full_val_int,COUNT_WIDTH);

 --prog_full_val <= mem_depth_f - (PROG_FULL_THRESH_BIN*wr_loops_b/4) + to_unsigned(m_full,COUNT_WIDTH-1);
   prog_full_val <= vc_val2(12 downto 0);

--  VC : process begin
--    if (vc_val2 >= mem_depth_f) then 
--       prog_full_val <= "1111111110000";
--    else 
--       prog_full_val <= vc_val2(12 downto 0);
--    end if;
--    wait on vc_val2;
...
-- eccparity is flopped
    synd_wr <= fn_ecc(c_encode, DIN_in, DINP_in);
