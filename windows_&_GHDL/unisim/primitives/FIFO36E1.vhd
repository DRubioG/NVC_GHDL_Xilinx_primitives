...
--    03/26/14 - Balanced all iniputs with xor (CR778933).
...
--    08/26/21 - Almost Empty behavior updated to match HW. 
-- End Revision
...
-- CR 182616 fix
   ...
   signal prev_time_rdclk : time := 0 ps;
   signal prev_time_wrclk : time := 0 ps;
...
--  variable Message : LINE;
  ...
  begin
      ...
      if (rising_edge(RDCLK_dly)) then
          ...
          if (count_freq_rdclk_var = 150) then
              rise_rdclk_var := now;
          elsif (count_freq_rdclk_var = 151) then
              period_rdclk_var := now - rise_rdclk_var;
          end if;
          
          if (count_freq_rdclk_var >= 151 and count_freq_wrclk >= 151 and RST_dly = '0' and GSR_dly = '0') then
  ...
  prcs_check_almost_wrclk:process(WRCLK_dly, count_freq_wrclk_reset)
    ...
    begin
      ...
      if (rising_edge(WRCLK_dly)) then
        ...
        if (count_freq_wrclk_var = 150) then
          rise_wrclk_var := now;
        elsif (count_freq_wrclk_var = 151) then
      ...
      if (now > 0 ps and (RDEN_dly = '1' or GSR_dly = '0')) then
        wait until (rising_edge(RDCLK_dly));
          if (first_rst_flag = '0' and RDEN_dly = '1') then
            assert false
              report "A RESET cycle must be observed before the first use of the FIFO instance."
              severity Error;
          end if;
      end if;
      wait on RDEN_dly;
...
S7_read_write : if (SIM_DEVICE = "7SERIES") generate
  ...
  prcs_sync_clk_async_mode : process(time_rdclk, time_wrclk)
    begin
      if (((time_rdclk - time_wrclk = 0 ps and prev_time_rdclk - prev_time_wrclk = 0 ps) or (time_wrclk - time_rdclk = 0 ps and prev_time_wrclk - prev_time_rdclk = 0 ps)) and now /= 0 ps) then
       ...
       if (sync_clk_async_mode = '1') then
        ...
        if(fwft = '0') then
         ...
         if((((rdcount_var + ae_empty) > wr_addr_var) and (rdcount_flag_var = awr_flag_var)) or (((rdcount_var + ae_empty) > (wr_addr_var + addr_limit)) and (rdcount_flag_var /= awr_flag_var))) then    
            ...
            almostempty_int(1) := '1';
            almostempty_int(0) := '1';
         elsif(almostempty_int(1)  = '0') then
           -- added to match verilog
           if (rdcount_var <= rdcount_var + ae_empty or rdcount_flag_var /= awr_flag_var) then
            almostempty_int(3) :=  almostempty_int(0);
            almostempty_int(0) :=  '0';
  ...
  begin
	...
	if (sync = '1') then
      ...
      if (sync_clk_async_mode = '1') then
        ...
        if((((rdcount_var + addr_limit) <= (wr_addr_var + ae_full + 1)) and (rdcount_flag_var = awr_flag_var)) or ((rdcount_var <= (wr_addr_var + ae_full + 1)) and (rdcount_flag_var /= awr_flag_var))) then    
          almostfull_int(3) := '1';
          almostfull_int(2) := '1';
          almostfull_int(1) := '1';
          almostfull_int(0) := '1';
...
architecture FIFO36E1_V of FIFO36E1 is
  ...
  signal IS_RDCLK_INVERTED_reg : std_ulogic := TO_X01(IS_RDCLK_INVERTED);
  signal IS_RDEN_INVERTED_reg : std_ulogic := TO_X01(IS_RDEN_INVERTED);
  signal IS_RSTREG_INVERTED_reg : std_ulogic := TO_X01(IS_RSTREG_INVERTED);
  signal IS_RST_INVERTED_reg : std_ulogic := TO_X01(IS_RST_INVERTED);
  signal IS_WRCLK_INVERTED_reg : std_ulogic := TO_X01(IS_WRCLK_INVERTED);
  signal IS_WREN_INVERTED_reg : std_ulogic := TO_X01(IS_WREN_INVERTED);
  signal DI_in : std_logic_vector(63 downto 0);
  signal DIP_in : std_logic_vector(7 downto 0);
