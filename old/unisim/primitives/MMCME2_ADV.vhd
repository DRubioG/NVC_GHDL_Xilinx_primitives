...
entity MMCME2_ADV is
  ...
  if (clkdiv >= 1) then
     ...
     clk_dly_int := real2int (clk_dly_rl + 0.0625);

     if (clk_dly_int > 63) then
        Write ( Message, string'(" Warning : [Unisim MMCME2_ADV-102] Attribute "));
     ...
     clkpm_sel_i := real2int((clk_dly_rem + 0.0625) * 8.0) mod 8;
     clkpm_sel_rl := real(clkpm_sel_i);
     clkpm_sel := STD_LOGIC_VECTOR(TO_UNSIGNED(clkpm_sel_i, 3));
     ...
     if (((clk_ps_rl- clk_ps) > 0.001) or ((clk_ps_rl- clk_ps) < -0.001)) then
        Write ( Message, string'("Warning : [Unisim MMCME2_ADV-103] Attribute "));
        Write ( Message, clk_ps_name );
        Write ( Message, string'(" of MMCME2_ADV is set to "));
        Write ( Message, clk_ps);
  ...
  begin
     ...
     if (clk_dly_int > 63) then
        Write ( Message, string'("Warning : [Unisim MMCME2_ADV-104] Attribute "));
        Write ( Message, clk_ps_name );
        Write ( Message, string'(" of MMCME2_ADV is set to "));
        Write ( Message, clk_ps);
        Write ( Message, string'(". Required phase shifting can not be reached since it is over the maximum phase shifting ability of MMCME2_ADV"));
     ...
     if (((clk_ps_rl- clk_ps) > 0.001) or ((clk_ps_rl- clk_ps) < -0.001)) then
        Write ( Message, string'(" Warning : [Unisim MMCME2_ADV-105] Attribute "));
        Write ( Message, clk_ps_name );
        Write ( Message, string'(" of MMCME2_ADV is set to "));
        Write ( Message, clk_ps);
        Write ( Message, string'(". Real phase shifting is "));
  ...
  begin
    ...
    if ((CLKOUT_DUTY_CYCLE > CLK_DUTY_CYCLE_CHK) or (CLKOUT_DUTY_CYCLE < CLK_DUTY_CYCLE_MIN_rnd)) then 
      Write ( Message, string'(" Warning : [Unisim MMCME2_ADV-107] "));
    ...
    if (duty_cycle_valid /= '1') then
      Write ( Message, string'(" Warning : [Unisim MMCME2_ADV-108] "));
      Write ( Message, CLKOUT_DUTY_CYCLE_N);
...
-- Input/Output Pin signals
       ...
       -- constant D_MAX : integer := 128;
        ...
        signal period_vco_frl : real := 0.0;
  ...
  signal drp_updt : bit := '0';
        ...
        signal IS_PSEN_INVERTED_BIN : std_ulogic := TO_X01(IS_PSEN_INVERTED);
        signal IS_PSINCDEC_INVERTED_BIN : std_ulogic := TO_X01(IS_PSINCDEC_INVERTED);
  ...
  begin
        ...
        dclk_in <= DCLK;
        psclk_in <= PSCLK;
        psen_in <= PSEN xor IS_PSEN_INVERTED_BIN;
        psincdec_in <= PSINCDEC xor IS_PSINCDEC_INVERTED_BIN;
   ...
   begin  -- process drp_monitor
     ...
     if (rising_edge(dclk_in)) then
      ...
      begin
           if((COMPENSATION /= "ZHOLD") and (COMPENSATION /= "zhold") and
             ...
             assert FALSE report " Error : [Unisim MMCME2_ADV-109] The Attribute COMPENSATION must be set to ZHOLD or BUF_IN or  EXTERNAL or INTERNAL." severity error;
         end if;
         ...
         if((BANDWIDTH /= "HIGH") and (BANDWIDTH /= "high") and
             ...
             assert FALSE report "Error : [Unisim MMCME2_ADV-110] BANDWIDTH  is not HIGH, LOW, OPTIMIZED." severity error;
         end if;
       ...
       if (CLKFBOUT_USE_FINE_PS = FALSE or CLKFBOUT_USE_FINE_PS = false) then
            ...
            assert FALSE report " Error : [Unisim MMCME2_ADV-111] The Attribute CLKFBOUT_USE_FINE_PS must be set to FALSE or TRUE." severity error;
       end if;
       ...
       if (CLKOUT0_USE_FINE_PS = FALSE or CLKOUT0_USE_FINE_PS = false) then
            ...
            assert FALSE report " Error : [Unisim MMCME2_ADV-112] The Attribute CLKOUT0_USE_FINE_PS must be set to FALSE or TRUE." severity error;
       end if;
       ...
       if (CLKOUT1_USE_FINE_PS = FALSE or CLKOUT1_USE_FINE_PS = false) then
            ...
            assert FALSE report " Error : [Unisim MMCME2_ADV-113] The Attribute CLKOUT1_USE_FINE_PS must be set to FALSE or TRUE." severity error;
       end if;
       ...
       if (CLKOUT2_USE_FINE_PS = FALSE or CLKOUT2_USE_FINE_PS = false) then
            ...
            assert FALSE report " Error : [Unisim MMCME2_ADV-114] The Attribute CLKOUT2_USE_FINE_PS must be set to FALSE or TRUE." severity error;
       end if;
       ...
       if (CLKOUT3_USE_FINE_PS = FALSE or CLKOUT3_USE_FINE_PS = false) then
            ...
            assert FALSE report " Error : [Unisim MMCME2_ADV-115] The Attribute CLKOUT3_USE_FINE_PS must be set to FALSE or TRUE." severity error;
       end if;
       ...
       if (CLKOUT4_USE_FINE_PS = FALSE or CLKOUT4_USE_FINE_PS = false) then
            ...
            assert FALSE report " Error : [Unisim MMCME2_ADV-116] The Attribute CLKOUT4_USE_FINE_PS must be set to FALSE or TRUE." severity error;
       end if;
       ...
       if (CLKOUT5_USE_FINE_PS = FALSE or CLKOUT5_USE_FINE_PS = false) then
            ...
            assert FALSE report " Error : [Unisim MMCME2_ADV-117] The Attribute CLKOUT5_USE_FINE_PS must be set to FALSE or TRUE." severity error;
       end if;
       ...
       if (CLKOUT6_USE_FINE_PS = FALSE or CLKOUT6_USE_FINE_PS = false) then
            ...
            assert FALSE report " Error : [Unisim MMCME2_ADV-118] The Attribute CLKOUT6_USE_FINE_PS must be set to FALSE or TRUE." severity error;
       end if;
       ...
       if (CLKOUT4_CASCADE = FALSE or CLKOUT4_CASCADE = false) then
           ...
           assert FALSE report " Error : [Unisim MMCME2_ADV-119] The Attribute CLKOUT4_CASCADE must be set to FALSE or TRUE." severity error;
       end if;
      ...
      if((STARTUP_WAIT = FALSE) or (STARTUP_WAIT = false)) then
        ...
        assert FALSE report "Error : [Unisim MMCME2_ADV-120] STARTUP_WAIT must be set to FALSE, TRUE." severity error;
      end if;
    ...
    if (clk0_sodd_and_frac >= 1 and  clk0_sodd_and_frac <= 8) then
       ...
       if (CLKOUT0_DIVIDE_F_RND < 0.000 or CLKOUT0_DIVIDE_F_RND > 128.000) then
          assert FALSE report "Error : [Unisim MMCME2_ADV-121] CLKOUT0_DIVIDE_F is not in range 1.000 to 128.000." severity error;
       end if;

       if (CLKOUT0_DIVIDE_F_RND > 1.0000 and  CLKOUT0_DIVIDE_F_RND < 2.0000) then
          assert FALSE report "Error : [Unisim MMCME2_ADV-122] CLKOUT0_DIVIDE_F is in range of greater than 1.0000 and less than 2.0000, which is not allowed." severity error;
       end if;

        if ((CLKOUT0_PHASE < -360.0) or (CLKOUT0_PHASE > 360.0)) then
            assert FALSE report "Error : [Unisim MMCME2_ADV-123] CLKOUT0_PHASE is not in range -360.0 to 360.0" severity error;
        end if;

       if (clk0_frac_en_v = '0') then
        if ((CLKOUT0_DUTY_CYCLE < 0.001) or (CLKOUT0_DUTY_CYCLE > 0.999)) then
           assert FALSE report "Error : [Unisim MMCME2_ADV-124] CLKOUT0_DUTY_CYCLE is not real in range 0.001 to 0.999 pecentage."severity error;
        end if;
       else
         if (CLKOUT0_DUTY_CYCLE > 0.5 or CLKOUT0_DUTY_CYCLE < 0.5) then
            assert FALSE report "Error : [Unisim MMCME2_ADV-125] CLKOUT0_DUTY_CYCLE should be set to 0.5 when CLKOUT0_DIVIDE_F has fraction part." severity error;
        end if;
       end if;

        case CLKOUT1_DIVIDE is
           when   1 to 128 => NULL ;
           when others  =>  assert FALSE report "Error : [Unisim MMCME2_ADV-126] CLKOUT1_DIVIDE is not in range 1 to 128." severity error;
        end case;

         if ((CLKOUT1_PHASE < -360.0) or (CLKOUT1_PHASE > 360.0)) then
            assert FALSE report "Error : [Unisim MMCME2_ADV-127] CLKOUT1_PHASE is not in range -360.0 to 360.0" severity error;
         end if;

         if ((CLKOUT1_DUTY_CYCLE < 0.001) or (CLKOUT1_DUTY_CYCLE > 0.999)) then
             assert FALSE report "Error : [Unisim MMCME2_ADV-128] CLKOUT1_DUTY_CYCLE is not real in range 0.001 to 0.999 pecentage."severity error;
         end if;

         case CLKOUT2_DIVIDE is
           when   1 to 128 => NULL ;
           when others  =>  assert FALSE report "Error : [Unisim MMCME2_ADV-129] CLKOUT2_DIVIDE is not in range 1 to 128." severity error;
         end case;

         if ((CLKOUT2_PHASE < -360.0) or (CLKOUT2_PHASE > 360.0)) then
            assert FALSE report "Error : [Unisim MMCME2_ADV-130] CLKOUT2_PHASE is not in range -360.0 to 360.0" severity error;
         end if;

         if ((CLKOUT2_DUTY_CYCLE < 0.001) or (CLKOUT2_DUTY_CYCLE > 0.999)) then
             assert FALSE report "Error : [Unisim MMCME2_ADV-131] CLKOUT2_DUTY_CYCLE is not real in range 0.001 to 0.999 pecentage."severity error;
         end if;

         case CLKOUT3_DIVIDE is
           when   1 to 128 => NULL ;
           when others  =>  assert FALSE report "Error : [Unisim MMCME2_ADV-132] CLKOUT3_DIVIDE is not in range 1...128." severity error;
         end case;
         ...
         if ((CLKOUT3_DUTY_CYCLE < 0.001) or (CLKOUT3_DUTY_CYCLE > 0.999)) then
             assert FALSE report "Error : [Unisim MMCME2_ADV-133] CLKOUT3_DUTY_CYCLE is not real in range 0.001 to 0.999 pecentage."severity error;
         end if;

         case CLKOUT4_DIVIDE is
           when   1 to 128 => NULL ;
           when others  =>  assert FALSE report "Error : [Unisim MMCME2_ADV-134] CLKOUT4_DIVIDE is not in range 1 to 128." severity error;
         end case;

         if ((CLKOUT4_PHASE < -360.0) or (CLKOUT4_PHASE > 360.0)) then
            assert FALSE report "Error : [Unisim MMCME2_ADV-135] CLKOUT4_PHASE is not in range -360.0 to 360.0" severity error;
         end if;

         if ((CLKOUT4_DUTY_CYCLE < 0.001) or (CLKOUT4_DUTY_CYCLE > 0.999)) then
             assert FALSE report "Error : [Unisim MMCME2_ADV-136] CLKOUT4_DUTY_CYCLE is not real in range 0.001 to 0.999 pecentage."severity error;
         end if;

           case CLKOUT5_DIVIDE is
             when   1 to 128 => NULL ;
             when others  =>  assert FALSE report "Error : [Unisim MMCME2_ADV-137] CLKOUT5_DIVIDE is not in range 1...128." severity error;
           end case;
           if ((CLKOUT5_PHASE < -360.0) or (CLKOUT5_PHASE > 360.0)) then
             assert FALSE report "Error : [Unisim MMCME2_ADV-138] CLKOUT5_PHASE is not in range 360.0 to 360.0" severity error;
           end if;
           if ((CLKOUT5_DUTY_CYCLE < 0.001) or (CLKOUT5_DUTY_CYCLE > 0.999)) then
             assert FALSE report "Error : [Unisim MMCME2_ADV-139] CLKOUT5_DUTY_CYCLE is not real in range 0.001 to 0.999 pecentage."severity error;
           end if;

         case CLKOUT6_DIVIDE is
           when   1 to 128 => NULL ;
           when others  =>  assert FALSE report "Error : [Unisim MMCME2_ADV-140] CLKOUT6_DIVIDE is not in range 1 to 128." severity error;
         end case;
         if ((CLKOUT6_PHASE < -360.0) or (CLKOUT6_PHASE > 360.0)) then
            assert FALSE report "Error : [Unisim MMCME2_ADV-141] CLKOUT6_PHASE is not in range 360.0 to 360.0" severity error;
         end if;
         if ((CLKOUT6_DUTY_CYCLE < 0.001) or (CLKOUT6_DUTY_CYCLE > 0.999)) then
            assert FALSE report "Error : [Unisim MMCME2_ADV-141] CLKOUT6_DUTY_CYCLE is not real in range 0.0 to 1.0 pecentage."severity error;
         end if;

         if (CLKFBOUT_MULT_F_RND < 2.000 or CLKFBOUT_MULT_F_RND > 64.000) then 
          assert FALSE report "Error : [Unisim MMCME2_ADV-142] CLKFBOUT_MULT is not in range 2.000 to 64.000 ." severity error;
         end if; 

       if ( CLKFBOUT_PHASE < -360.0 or CLKFBOUT_PHASE > 360.0 ) then
             assert FALSE report "Error : [Unisim MMCME2_ADV-143] CLKFBOUT_PHASE is not in range -360.0 to 360.0" severity error;
       end if;

       case DIVCLK_DIVIDE is
         when    1  to 106 => NULL;
         when others  =>  assert FALSE report "Error : [Unisim MMCME2_ADV-144] DIVCLK_DIVIDE is not in range 1 to 106." severity error;
       end case;

       if ((REF_JITTER1 < 0.0) or (REF_JITTER1 > 0.999)) then
         assert FALSE report "Error : [Unisim MMCME2_ADV-145] REF_JITTER1 is not in range 0.0 ... 1.0." severity error;
       end if;

       if ((REF_JITTER2 < 0.0) or (REF_JITTER2 > 0.999)) then
         assert FALSE report "Error : [Unisim MMCME2_ADV-146] REF_JITTER2 is not in range 0.0 ... 1.0." severity error;
       end if;
    ...
    begin
      ...
      if (NOW > 1 ps  and  rst_in = '0' and (clkinsel_tmp = '0' or clkinsel_tmp = '1')) then
          assert false report
            "Error : [Unisim MMCME2_ADV-147] input clock can only be switched when RST=1.  CLKINSEL is changed when RST low, which should be changed at RST high."
          severity error;
      end if;
        ...
        Write ( Message, string'(" Error : [Unisim MMCME2_ADV-148] The attribute CLKIN1_PERIOD is set to "));
        ...
        Write ( Message, string'(" Error : [Unisim MMCME2_ADV-149] The attribute CLKIN2_PERIOD is set to "));
        ...
        if (period_clkin > 0.000) then
          ...
          if ((clkvco_freq_init_chk > VCOCLK_FREQ_MAX) or (clkvco_freq_init_chk < VCOCLK_FREQ_MIN)) then
           Write ( Message, string'(" Error : [Unisim MMCME2_ADV-150] The calculation of VCO frequency="));
    ...
    rst_in_o <= rst_in or rst_clkfbstopped or rst_clkinstopped or clkpll_jitter_unlock;
      ...
      if (rising_edge(rst_input)) then
         ...
         if (rst_ht < 1.5 ns and rst_ht > 0 ps) then
           if (rst_input_r_h = '1' and  pwrdwn_in1_h = '1') then
            assert false report
               "Error : [Unisim MMCME2_ADV-152] RST and PWRDWN must be asserted at least for 1.5 ns."
            severity error;
           elsif (rst_input_r_h = '1' and  pwrdwn_in1_h = '0') then
            assert false report
               "Error : [Unisim MMCME2_ADV-153] RST  must be asserted at least for 1.5 ns."
            severity error;
           elsif (rst_input_r_h = '0' and  pwrdwn_in1_h = '1') then
            assert false report
               "Error :  [Unisim MMCME2_ADV-154] PWRDWN must be asserted at least for 1.5 ns."
            severity error;
           end if;
         end if;
      end if;
...
----
  ...
  case clkfb_div_fint is
    ...
    if (den_in = '1') then
       ...
       if (valid_daddr and ( address = 116  or address = 78 or address = 79 or
               ...
               Write ( Message, string'(" Warning :  [Unisim MMCME2_ADV-155] Address DADDR="));
       ...
       if (dwe_in = '1')  then
          ...
          if (rst_input = '1') then
             ...
             if (daddr_in = "0001001") then -- 9
                 ...
                 if (di_in(11) = '1') then
                    if (clk0_lti = "1000000") then
                      clk0_lt <= "0000000";
                      clk0_lti := "0000000";
                    end if;
                    if (clk0_hti = "1000000") then
                      clk0_ht <= "0000000";
                      clk0_hti := "0000000";
                    end if;
                    clk0_frac_en <= '1';
                    ...
                    if (clk0_lti = "0000000") then
                      clk0_lt <= "1000000";
                      clk0_lti := "1000000";
                    end if;
                    if (clk0_hti = "0000000") then
                      clk0_ht <= "1000000";
                      clk0_hti := "1000000";
                    end if;
                    clk0_frac_en_v := '0';
             ...
             if (daddr_in = "0010100") then -- 20
               ...
               if (((clkfbm1_divi) > M_MAX or (clkfbm1_divi) < M_MIN ) and (clkfb_frac_en = '0')) then
               ...
               Write ( Message, address);
               Write ( Message, string'(" is CLKFBOUT clock divider of MMCME2_ADV and over the "));
               Write ( Message, M_MIN);
               Write ( Message, string'(" to "));
               Write ( Message, M_MAX);
             ...
             if (clkfb_frac_en_v = '0') then
              ...
              if (clk_div > D_MAX or (clk_div < D_MIN and clkind_nocnti = '0')) then
               ...
               Write ( Message, string'(") at Address DADDR = "));
               Write ( Message, address);
               Write ( Message, string'(" is input clock divider of MMCME2_ADV and over the "));
               Write ( Message, D_MIN);
               Write ( Message, string'(" to "));
               Write ( Message, D_MAX);
               Write ( Message, string'(" range"));
              ...
              assert false report " Error : [Unisim MMCME2_ADV-158] RST is low at MMCME2_ADV. RST need to be high when change MMCME2_ADV paramters through DRP. " severity error;
          end if; -- end rst
  ...
  clkfb_frac_para_p : process (clkfbm1_ht, clkfbm1_lt, clkfb_frac, clkfb_frac_en, clkfbm1pmf_sel, clkfbm1pmr_sel, clkfbm1_div)
  ...
  begin
    if (clkfb_frac_en = '1') then
     ...
     mf_product <=  clkfbm1_div * 8; 
     mf_product1 <= (clkfbm1_div * 8) - 1; 
  ...
  clkfb_frac_ht_lt_p : process (period_vco_frl, clkfbm1_fht, clkfbm1_flt, clkfbm1_rsel, clkfbm1_fsel) begin
    clkfb_frac_ht <= (period_vco_frl * real(clkfbm1_fht) + (period_vco_frl * real(clkfbm1_rsel)) / 8.0) * 1 ps;
    clkfb_frac_lt <= (period_vco_frl * real(clkfbm1_flt) + (period_vco_frl * real(clkfbm1_fsel)) / 8.0) * 1 ps;
  end process;

  clk0_frac_ht_lt_p : process(period_vco_frl, clk0_fht, clk0_flt, clk0pm_fsel, clk0pm_rsel) begin
    clk0_frac_ht <= (period_vco_frl * real(clk0_fht) + (period_vco_frl * real(clk0pm_rsel)) / 8.0) * 1 ps;
    clk0_frac_lt <= (period_vco_frl * real(clk0_flt) + (period_vco_frl * real(clk0pm_fsel)) / 8.0) * 1 ps;
  end process;
    ...
    begin
      ...
      if (rst_in = '1') then
        ...
        if (fps_en = 1)  then
          ...
          if (psen_in = '1')  then
            if (psen_w = '1') then
              assert false report " Error : [Unisim MMCME2_ADV-159] PSEN on MMCME2_ADV is active more than 1 PSCLK period. PSEN must be active for only one PSCLK period. "
              severity warning;
            end if;
            psen_w <= '1';
            if (ps_lock = '1') then
              assert false report " Warning : [Unisim MMCME2_ADV-160] Please wait for PSDONE signal before adjusting the Phase Shift. " 
              severity warning;
    ...
    begin
      if (rst_in = '1' or rst_clkinsel_flag = '1') then
        ...
        if ( (pll_unlock1 = '0' or unlock_recover = '1') and  (clkin_lock_cnt < lock_cnt_max) and fb_delay_found = '1' ) then
        ...
        if (( clkin_lock_cnt >= PLL_LOCK_TIME and pll_unlock = '0') or
            (unlock_recover = '1' and clkin_lock_cnt > lock_cnt_max-2)) then
            pll_locked_tm <= '1' after 1 ps;
        end if;
  ...
  if (rising_edge(pll_locked_tmp1)) then
    if (clkinsel_in = '0') then
       ...
       if (period_avg > pchk_tmp1 or  period_avg < pchk_tmp2) then
         assert false report "Warning : [Unisim MMCME2_ADV-161] input CLKIN2 period and attribute CLKIN2_PERIOD are not same." severity Warning ;
       end if;
    else 
       pchk_tmp1 := CLKIN1_PERIOD * 1100 ps;
       pchk_tmp2 := CLKIN1_PERIOD * 900 ps;
       if (period_avg > pchk_tmp1 or  period_avg < pchk_tmp2) then
         assert false report "Warning : [Unisim MMCME2_ADV-162] input CLKIN1 period and attribute CLKIN1_PERIOD are not same." severity Warning ;
       end if;
    end if;
  end if;
  end process;
    ...
    PLL_LOCK_P1 : process (pll_locked_tmp1, rst_in_o)
    begin
      if (rst_in_o = '1') then
         pll_locked_tmp2 <= '0';
      elsif (pll_locked_tmp1 = '0') then
         pll_locked_tmp2 <=  pll_locked_tmp1;
      elsif (rising_edge(pll_locked_tmp1)) then
   ...
   if (rst_in = '1') then
    ...
    begin
      if (init_done = '1' and fb_delay_found = '1') then
       ...
       md_product_v := real2int(clkfbm1_div_t * clkind_div_rl + 0.5);
       ...
       period_vco_frl <= period_vco_tmp0;
  ...
  begin
   ...
   if (rising_edge(ps_lock_dly)) then
     if ((period_ps - period_ps_old) > period_vco_half ) then
      ...
      begin
      if ((fps_en = 1) and (fps_clk_en = 1)) then
        if (ps_in_ps = 0 ) then
           clkout_ps_i := clkvco;
  ...
  --
      ...
      if (rising_edge(rst_in))  then
        ...
        if (fb_delay_found_tmp = '0') then
          ...
          if (fb_delay_tmp > fb_delay_max) then
            Write ( Message, string'(" Warning : [Unisim MMCME2_ADV-163] The feedback delay is "));
    ...
    fb_delay_found_P : process(clkfb_tst, rst_in)
    begin
      if (rising_edge(rst_in)) then
        fb_delay_found <= '0';
      elsif (falling_edge(clkfb_tst)) then
  ...
  begin
    if (rst_in = '1') then
       ...
       if (clkin_hold_f = 1) then
         ...
         assert FALSE  report " Warning : [Unisim MMCME2_ADV-164] input CLKIN1 of MMCME2_ADV is stopped. Reset is required for MMCME2_ADV when input clock returns." severity error;
         else
         assert FALSE report " Warning : [Unisim MMCME2_ADV-165] input CLKIN2 of MMCME2_ADV  is stopped. Reset is required for MMCME2_ADV when input clock returns." severity error;
         end if;
       end if;
    end if;
  ...
  clkfb_stopped_p : process(clk_osc, rst_in, clkfb_p, pll_unlock)
  begin
     if (rst_in = '1' or clkfb_p = '1' or pll_unlock = '1') then
       clkfbstopped_out <= '0';
       clkfb_lost_cnt <= 0;
     elsif (rising_edge(clk_osc)) then
