...
architecture IBUFE3_V of IBUFE3 is
  ...
  begin
    int := 0;
    for j in SLV'high downto SLV'low loop
      int := int * 2;
      if SLV(j) = '1' then
        int := int + 1;
      end if;
...
-- Parameter encodings and registers
  constant CCIO_EN_FALSE : std_ulogic := '1';
  constant CCIO_EN_TRUE : std_ulogic := '0';
  ...
  CCIO_EN_BIN <= 
      CCIO_EN_TRUE when (CCIO_EN = "TRUE") else
      CCIO_EN_FALSE when (CCIO_EN = "FALSE") else
      CCIO_EN_TRUE;
  ...
  begin
    -------- CCIO_EN check
      if((xil_attr_test) or
        ...
        Write ( Message, string'("-101] CCIO_EN attribute is set to """));
        Write ( Message, string'(CCIO_EN));
